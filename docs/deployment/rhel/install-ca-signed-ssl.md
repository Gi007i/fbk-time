# Reguläres SSL-Zertifikat (CSR) erstellen (RHEL)
...
# SELinux Tools (falls nicht vorhanden)
...
```bash
sudo openssl genrsa -out /etc/pki/tls/private/fbk-time.key 4096
...
```bash
sudo openssl req -new -key /etc/pki/tls/private/fbk-time.key \
  -out /etc/pki/tls/certs/fbk-time.csr
...
```bash
# CSR-Inhalt anzeigen
sudo openssl req -in /etc/pki/tls/certs/fbk-time.csr -noout -text

# Subject anzeigen
sudo openssl req -in /etc/pki/tls/certs/fbk-time.csr -noout -subject

# SANs anzeigen (falls vorhanden)
sudo openssl req -in /etc/pki/tls/certs/fbk-time.csr -noout -text | grep -A1 "Subject Alternative Name"

# CSR-Signatur verifizieren
sudo openssl req -in /etc/pki/tls/certs/fbk-time.csr -noout -verify
```
...
sudo cat /etc/pki/tls/certs/fbk-time.csr
```
...
```bash
sudo mv fbk-time.crt /etc/pki/tls/certs/fbk-time.crt
sudo cat intermediate.crt root.crt > /etc/pki/tls/certs/ca-bundle.crt
```
...
```nginx
# /etc/nginx/conf.d/fbk-time.conf
...
sudo openssl req -new \
  -key /etc/pki/tls/private/fbk-time.key \
  -out /etc/pki/tls/certs/fbk-time-renewal.csr \
  -config /tmp/csr.cnf

# CSR an CA senden, neues Zertifikat erhalten
# Zertifikat ersetzen
sudo mv new-fbk-time.crt /etc/pki/tls/certs/fbk-time.crt
...
0 3 * * * /usr/bin/openssl x509 -checkend 2592000 -noout -in /etc/pki/tls/certs/fbk-time.crt || echo "SSL-Zertifikat läuft in weniger als 30 Tagen ab!" | mail -s "SSL-Warnung" admin@example.com
```
...
### SELinux blockiert Zugriff
**Problem:** SELinux verhindert Zugriff auf Zertifikate
**Lösung:**
```bash
sudo ausearch -m AVC -ts recent | grep nginx
sudo restorecon -Rv /etc/pki/tls/
