# Self-Signed SSL-Zertifikat erstellen (Debian/Ubuntu)
...
- **Zertifikat:** `/etc/ssl/certs/fbk-time.crt`
- **Private Key:** `/etc/ssl/private/fbk-time.key`
...
sudo openssl req -x509 -nodes -days 365 -newkey rsa:4096 \
  -keyout /etc/ssl/private/fbk-time.key \
  -out /etc/ssl/certs/fbk-time.crt \
...
sudo chown root:root /etc/ssl/certs/fbk-time.crt
```
...
sudo openssl rsa -in /etc/ssl/private/fbk-time.key -check
...
```nginx
# In /etc/nginx/sites-available/fbk-time
...
sudo rm /etc/ssl/certs/fbk-time.crt
...
0 3 * * * /usr/bin/openssl x509 -checkend 2592000 -noout -in /etc/ssl/certs/fbk-time.crt || (/usr/bin/openssl req -new -x509 -days 365 -key /etc/ssl/private/fbk-time.key -out /etc/ssl/certs/fbk-time.crt -subj "/C=DE/ST=NRW/L=Koeln/O=MeineFirma/OU=IT/CN=absence.example.com" && /usr/sbin/nginx -s reload)
```
