...
(function() {
    ...
    function renderCell(date, col) {
        ...
        if (calendarData.holidays[dateStr]) {
            ...
            html += '<span data-tooltip="' + holidayName + '"><mark class="absence-item"><small>' + holidayName + '</small></mark></span>';
        }
        ...
        for (var i = 0; i < Math.min(dayOccurrences.length, maxShow); i++) {
            ...
            var detailUrl = occ.isRecurring
                ? '/absences/' + occ.absenceId + '/occurrence/' + occ.date
                : '/absences/' + occ.absenceId;

            html += '<span data-tooltip="' + tooltip + '"><a href="' + detailUrl + '" class="absence-item category-' + occ.categoryId + '">' + label + '</a></span>';
        }
